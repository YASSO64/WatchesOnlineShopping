 ...
 * @author Rania
 */
@WebServlet(name = "EditProductServ", urlPatterns = {"/EditProductServ"})
public class EditProductServ extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            producDB.UpdateProduct(product);
            ProductBean product2=producDB.getProduct(id);
            RequestDispatcher fd=request.getRequestDispatcher("adminjsp/productprofile.jsp");
            fd.include(request, response);
        } catch (SQLException ex) {
            Logger.getLogger(EditProductServ.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(EditProductServ.class.getName()).log(Level.SEVERE, null, ex);
        }
